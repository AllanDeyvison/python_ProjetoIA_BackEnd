import os
import ollama
from models.history import (
    create_chat,
    update_chat,
    list_last_chats,
    get_chat_info,
    get_chat_messages,
    delete_conversation
)

LLM_MODEL = os.getenv('LLM_MODEL')

# Função auxiliar para gerar a resposta em streaming
def generate_streamed_response(messages):
    """
    Função geradora para consultar o modelo Ollama em modo de streaming.
    """
    try:
        # Usa o parâmetro 'stream=True' para obter a resposta em pedaços
        stream = ollama.chat(model=LLM_MODEL, messages=messages, stream=True)
        
        # O gerador vai 'yield' cada pedaço da resposta à medida que chega
        for chunk in stream: 
            # O 'chunk' é um dicionário, o texto está em ['content']
            yield chunk['message']['content']
            
    except Exception as e:
        print(f"Erro ao consultar o modelo: {e}")
        # Você pode 'yield' uma mensagem de erro para o frontend
        yield "Ocorreu um erro ao gerar a resposta."

def query_new_chat(user_id: str, query: str):
    """
    Inicia um novo chat e retorna um gerador para a resposta.
    """
    messages = [
        {
            "role": "system",
            'content': 'Você é o AprovIA um assistente especializado em resolver problemas matemáticos. Responda com o passo a passo de forma clara e concisa. Responda sempre em Português Brasileiro e organize em MarkDown. Nunca dê a resposta ao final, apenas o passo a passo. Se não souber a resposta, diga que não sabe. Caso a pergunta não tenha relação com matemática, diga que não sabe e não tente responder.',
        },
        {"role": "user", "content": query}
    ]
    
    # Cria o chat e salva a primeira mensagem do usuário imediatamente
    chat_id = create_chat(user_id, title=f'{query.capitalize()[:30]}...')
    update_chat(user_id, chat_id, "user", query)

    # Cria uma lista vazia para armazenar a resposta completa, 
    # pois o gerador 'generate_streamed_response' não salva a resposta.
    full_response_content = []

    def stream_and_save_response():
        # Este gerador vai ser usado na rota Flask
        for chunk in generate_streamed_response(messages):
            full_response_content.append(chunk)
            yield chunk
        
        # Salva a resposta completa no banco de dados após o stream terminar
        model_response = "".join(full_response_content)
        update_chat(user_id, chat_id, "assistant", model_response)

    return {
        "chat_id": chat_id,
        "resposta_stream": stream_and_save_response() # Retorna o gerador!
    }

def continue_chat(user_id: str, chat_id: str, query: str):
    """
    Continua um chat existente e retorna um gerador para a resposta.
    """
    chat = get_chat_messages(user_id, chat_id)
    messages = [
        {
            "role": "system", 
            "content": "Você é um assistente especializado em resolver problemas matemáticos. Responda com o passo a passo de forma clara e concisa. Responda sempre em Português Brasileiro e organize em MarkDown. Nunca dê a resposta ao final, apenas o passo a passo. Se não souber a resposta, diga que não sabe. Caso a pergunta não tenha relação com matemática, diga que não sabe e não tente responder. A partir disso continue a explicação matemática:"
        }
    ]
    messages.extend(chat["messages"][-10:])
    messages.append({"role": "user", "content": query})
    
    # Adiciona a nova mensagem do usuário no chat
    update_chat(user_id, chat_id, "user", query)
    
    full_response_content = []

    def stream_and_save_response():
        for chunk in generate_streamed_response(messages):
            full_response_content.append(chunk)
            yield chunk
            
        model_response = "".join(full_response_content)
        update_chat(user_id, chat_id, "assistant", model_response)

    return {
        "chat_id": chat_id,
        "resposta_stream": stream_and_save_response() # Retorna o gerador!
    }

# As funções add_messages, list_last_chats etc, não precisam de alteração.

def add_messages(user_id, chat_id, query, model_response):
    update_chat(user_id, chat_id, "user", query)
    update_chat(user_id, chat_id, "assistant", model_response)